...
namespace Fastcgipp
{
    ...
    /*
                ...
                bool kill)
        {
            ...
            std::unique_ptr<Request<charT>> request(new Request<charT>);
            request->configure(
                    id,
                    role,
                    kill,
...
*/    
    public:
        ...
        std::unique_ptr<Request_base> upgradeRequest(std::unique_ptr<Request_base> oldReq) {
            ...
            request->configureOp(
                    std::bind(&Transceiver::send, &this->Manager_base::m_transceiver, 
                                    std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
                    std::bind(&Manager_base::push, this, request->id(), std::placeholders::_1));
            vlog("exit %s\n", __func__);
            return request;
            /*
            return nullptr;
