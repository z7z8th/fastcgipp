...
public:
    Echo():
        Fastcgipp::Request<wchar_t>(10*1024*1024)
    {}
    Echo(Request<wchar_t>&& rhs) :
        Fastcgipp::Request<wchar_t>::Request(std::move(rhs))
    {
        Fastcgipp::Request<wchar_t>::m_maxPostSize = 10*1024*1024;
        vlog("%s m_maxPostSize %zd\n", __PRETTY_FUNCTION__, Fastcgipp::Request<wchar_t>::m_maxPostSize);
    }
    ~Echo() {
        vlog("%s\n", __PRETTY_FUNCTION__);
    }
